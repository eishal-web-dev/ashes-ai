...
def _defaults() -> dict[str, Any]:
    return {
        ...
        "plans": {
            ...
            "starter": {"enabled": True, "price_monthly": float(PLANS["starter"]["price_monthly_usd"])},
            "pro": {"enabled": True, "price_monthly": float(PLANS["pro"]["price_monthly_usd"])},
        },
    }
...
def get_billing_settings() -> dict[str, Any]:
    ...
    for key in ("free", "starter", "pro"):
        if key in saved_plans:
            base["plans"][key].update(saved_plans[key])
    return base
...
def update_billing_settings(currency: str, plans: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(currency_key) != 3 or not currency_key.isalpha():
        raise ValueError("Currency must be a 3-letter ISO currency code")

    current = get_billing_settings()
    ...
    for key in ("starter", "pro"):
        ...
        if "enabled" in incoming:
            normalized[key]["enabled"] = bool(incoming["enabled"])

    collection("billing_settings").update_one(
        {"key": "global"},
        {"$set": {"key": "global", "currency": currency_key, "plans": normalized}},
        upsert=True,
    )
...
def public_plan_catalog() -> list[dict[str, Any]]:
    ...
    for key in ("free", "starter", "pro"):
        base = dict(PLANS[key])
        configured = settings["plans"][key]
        base["enabled"] = bool(configured.get("enabled", True))
        base["currency"] = settings["currency"]
        base["price_monthly"] = float(configured.get("price_monthly", 0))
        rows.append(base)
    return rows
...
def checkout_price(plan_key: str) -> tuple[str, int]:
    ...
    if amount_minor <= 0:
        raise ValueError("Paid plan price must be greater than zero")
