...
import { ArrowLeft, BarChart3, Bell, Box, Building2, Camera, Check, ChefHat, ChevronRight, Download, ImagePlus, LogOut, Pencil, Plus, QrCode, ScanLine, Sparkles, Trash2, Upload, Utensils, WandSparkles, X } from 'lucide-react';
...
export default function BusinessDashboard({ onBack, onOpenProduct, business, user, onLogout, onBusinessUpdated }) {
  ...
  const loadDashboard = async () => {
    if (!slug) return;
    try {
      const [items, metrics, latestOrders, qrs, imports] = await Promise.all([
  ...
  const runMenuImport = async () => {
    if (!menuImportFile || !slug) { setError('Choose a clear photo of the menu card first.'); return; }
    setMenuImporting(true); setError(''); setMenuImportResult(null);
    try {
      const result = await importMenuCard(slug, menuImportFile);
  ...
  return <main className="business-shell" style={{ '--business-accent': business?.accent_color || '#ff2f9f' }}><div className="noise" /><div className="dashboard-layout">
    ...
    <section className="menu-import-panel glass-panel">
      ...
      {menuImportResult && <div className="menu-import-result"><Check size={19}/><div><strong>{menuImportResult.items_found} menu items created as drafts.</strong><span>Review names/prices, add a product photo to each item for 3D generation, then publish when ready.</span></div></div>}
