...
import { absoluteApiUrl, clearSession, createBusinessProduct, createTableQr, deleteBusinessProduct, getBusinessAnalytics, getBusinessOrders, getBusinessProducts, getTableQrs, updateBusinessProduct, updateBusinessProfile, updateOrderStatus, uploadBusinessLogo } from './api';
...
export default function BusinessDashboard({ onBack, onOpenProduct, business, user, onLogout, onBusinessUpdated }) {
  ...
  return <main className="business-shell" style={{ '--business-accent': business?.accent_color || '#ff2f9f' }}><div className="noise" /><div className="dashboard-layout">
    ...
    <section className="orders-panel glass-panel"><div className="panel-head"><div><span className="kicker">LIVE KITCHEN QUEUE</span><h2>Incoming table orders</h2></div><button className="secondary-btn" onClick={loadDashboard}>Refresh</button></div><div className="orders-list">{orders.length === 0 && <div className="empty-catalog">No orders yet.</div>}{orders.slice(0,8).map(order => <article className="order-card" key={order.id}><div className="order-card-top"><div><strong>#{order.id.slice(0,8).toUpperCase()}</strong><span>{order.table_code ? `Table ${order.table_code}` : 'No table'}{order.customer_name ? ` · ${order.customer_name}` : ''}</span></div><b>Rs {Number(order.total).toLocaleString()}</b></div><div className="order-items-mini">{(order.items || []).map(item => <span key={item.product_id}>{item.quantity}× {item.product_name}</span>)}</div><div className="order-status-actions">{['new','accepted','preparing','ready','served'].map(status => <button key={status} className={order.status === status ? 'active' : ''} onClick={() => changeOrderStatus(order.id,status)}>{status}</button>)}<button className="danger" onClick={() => changeOrderStatus(order.id,'cancelled')}>cancel</button></div></article>)}</div></section>

    <section className="table-qr-panel glass-panel"><div className="panel-head"><div><span className="kicker">TABLE ENTRY POINTS</span><h2>Create printable QR codes</h2></div></div><div className="table-qr-builder"><label><span>Table code</span><input value={tableCode} onChange={e => setTableCode(e.target.value.toUpperCase())} /></label><label><span>Open directly to product</span><select value={tableProductId} onChange={e=>setTableProductId(e.target.value)}><option value="">Table session only</option>{products.filter(p=>p.is_published).map(p=><option value={p.id} key={p.id}>{p.name}</option>)}</select></label><button className="primary-btn" onClick={addTableQr}><Plus size={16}/> Create table QR</button></div><div className="table-qr-grid">{tableQrs.map(qr=><article className="table-qr-card" key={qr.id}><img src={absoluteApiUrl(qr.qr_url)} alt={`QR for table ${qr.table_code}`} /><div><strong>Table {qr.table_code}</strong><span>{qr.public_url}</span><a className="secondary-btn" href={absoluteApiUrl(qr.qr_url)} download><Download size={15}/> Download QR</a></div></article>)}</div></section>

    <section className="catalog-panel glass-panel"><div className="panel-head"><div><span className="kicker">CATALOG CONTROL</span><h2>Products, drafts & publishing</h2></div><button className="text-btn" onClick={()=>setStep('add')}>Add new <ChevronRight size={15}/></button></div><div className="product-table">{products.length===0 && <div className="empty-catalog">No products yet.</div>}{products.map(product => <div className="product-manage-card" key={product.id}>{editing===product.id ? <div className="edit-product-grid"><input value={editForm.name} onChange={e=>setEditForm({...editForm,name:e.target.value})}/><input value={editForm.category} onChange={e=>setEditForm({...editForm,category:e.target.value})}/><input type="number" value={editForm.price} onChange={e=>setEditForm({...editForm,price:e.target.value})}/><input value={editForm.tags} onChange={e=>setEditForm({...editForm,tags:e.target.value})}/><div className="product-manage-actions"><button className="primary-btn" onClick={()=>saveEdit(product.id)}>Save</button><button className="secondary-btn" onClick={()=>{setEditing(null);setEditForm(null)}}>Cancel</button></div></div> : <><div className="product-thumb"><Box size={19}/></div><div className="product-meta"><strong>{product.name}</strong><span>{product.category} · Rs {Number(product.price).toLocaleString()}</span></div><span className={`publish-pill ${product.is_published ? 'published' : 'draft'}`}>{product.is_published ? 'Published' : 'Draft'}</span><span className={`status-pill ${product.status==='ready'?'ready':'processing'}`}>{product.status==='ready'?'3D Ready':product.status}</span><div className="product-manage-actions"><button className="secondary-btn" onClick={()=>togglePublish(product)}>{product.is_published?'Unpublish':'Publish'}</button><button className="row-action" onClick={()=>beginEdit(product)} title="Edit"><Pencil size={16}/></button>{product.is_published && <button className="row-action" onClick={()=>onOpenProduct?.(product.id)} title="Preview"><ChevronRight size={16}/></button>}<button className="row-action danger" onClick={()=>removeProduct(product)} title="Delete"><Trash2 size={16}/></button></div></>}</div>)}</div></section>
  </section></div></main>;
}
