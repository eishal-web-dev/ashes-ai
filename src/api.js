...
export async function uploadBusinessLogo(slug,file){const form=new FormData();form.append('logo',file);const business=await apiFetch(`/api/storage/businesses/${slug}/logo`,{method:'POST',body:form},true);updateStoredBusiness(business);return business}
export async function importMenuCard(slug,file){const form=new FormData();form.append('image',file);return apiFetch(`/api/storage/businesses/${slug}/import-menu-card`,{method:'POST',body:form},true)}
...
export async function getBusinessProducts(slug,includeUnpublished=false){return apiFetch(`/api/businesses/${slug}/products${includeUnpublished?'?include_unpublished=true':''}`,{},includeUnpublished)}
...
export async function updateBusinessProduct(slug,productId,values){return apiFetch(`/api/businesses/${slug}/products/${productId}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(values)},true)}
export async function deleteBusinessProduct(slug,productId){return apiFetch(`/api/storage/businesses/${slug}/products/${productId}`,{method:'DELETE'},true)}
export async function getOwnedProduct(slug,productId){return apiFetch(`/api/businesses/${slug}/products/${productId}`,{},true)}
export async function getProduct(productId){return apiFetch(`/api/products/${productId}`)}
export async function trackProductEvent(productId,eventType){if(!productId)return null;return apiFetch(`/api/products/${productId}/analytics`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({event_type:eventType})})}
...
export async function createBillingCheckout(slug,plan,successUrl=null,cancelUrl=null){return apiFetch(`/api/businesses/${slug}/billing/checkout`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({plan,success_url:successUrl,cancel_url:cancelUrl})},true)}
export async function getBillingCheckout(slug,intentId){return apiFetch(`/api/businesses/${slug}/billing/checkout/${intentId}`,{},true)}
export async function devCompleteBillingCheckout(slug,intentId){return apiFetch(`/api/businesses/${slug}/billing/checkout/${intentId}/dev-complete`,{method:'POST'},true)}
export async function getManualPaymentMethods(){return apiFetch('/api/billing/manual-methods')}
export async function submitManualPaymentProof(slug,values){const form=new FormData();form.append('plan',values.plan);form.append('method',values.method);form.append('transaction_reference',values.transaction_reference||'');form.append('note',values.note||'');form.append('receipt',values.receipt);return apiFetch(`/api/businesses/${slug}/billing/manual-proof`,{method:'POST',body:form},true)}
...
export async function setAdminBusinessStatus(businessId,action){return apiFetch(`/api/admin/businesses/${businessId}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({action})},true)}
...
export async function updateAdminBillingSettings(values){return apiFetch('/api/admin/billing/settings',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(values)},true)}
export async function getAdminManualPayments(){return apiFetch('/api/admin/manual-payments',{},true)}
export async function updateAdminManualPaymentSettings(values){return apiFetch('/api/admin/manual-payments/settings',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(values)},true)}
export async function reviewAdminManualPayment(proofId,action,note=''){return apiFetch(`/api/admin/manual-payments/${proofId}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,note})},true)}
export function absoluteApiUrl(path){if(!path)return null;if(/^https?:\/\//i.test(path))return path;return `${API_BASE}${path}`}
